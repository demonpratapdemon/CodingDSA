...
public class CourseSchedule {
	...
	public static void main(String[] args) {
		...
		int numCourses = 100, prerequisites[][] = { { 1, 0 }, { 2, 0 }, { 2, 1 }, { 3, 1 }, { 3, 2 }, { 4, 2 },
				{ 4, 3 }, { 5, 3 }, { 5, 4 }, { 6, 4 }, { 6, 5 }, { 7, 5 }, { 7, 6 }, { 8, 6 }, { 8, 7 }, { 9, 7 },
				{ 9, 8 }, { 10, 8 }, { 10, 9 }, { 11, 9 }, { 11, 10 }, { 12, 10 }, { 12, 11 }, { 13, 11 }, { 13, 12 },
	...
	public static boolean canFinish(int numCourses, int[][] prerequisites) {
		...
		boolean isCyclePresent = findCycle(adj);
		if (isCyclePresent)
			return false;
		return true;
	}

	private static boolean findCycle(ArrayList<ArrayList<Integer>> adj) {
		// TODO Auto-generated method stub
		int n = adj.size();
		...
		Arrays.fill(arr, -1);
		for (int i = 0; i < n; i++) {
			boolean check = dfs(adj, i, visited, arr);
			if (check)
				return true;
			arr[i] = 0;
		}
	...
	private static boolean dfs(ArrayList<ArrayList<Integer>> adj, int i, boolean[] visited, int[] arr) {
		// TODO Auto-generated method stub
		if (visited[i])
			return true;
		if (arr[i] == 0)
		...
		for (int k = 0; k < adj.get(i).size(); k++) {
			boolean check = dfs(adj, adj.get(i).get(k), visited, arr);
			if (check)
				return true;
		}
		visited[i] = false;
